...
- 📸 **照片打卡**：决定之后上传一张照片留证，可重新上传替换；统计网格里有 📷 角标（带评论数），点开即看大图
- 💬 **评论互动**：打卡照片更像一条「帖子」，大家可以在下面留言、删自己的评论，角标实时显示评论数
...
- 评论**限长 500 字**、只能删自己的、渲染时统一 `escapeHtml` 转义（不在入库时改写内容），杜绝存储型 XSS
...
├── routes/
│   ├── auth.js            # 注册 / 验证 / 登录 / 登出
│   ├── decision.js        # 转轮盘 / 手动选择 / 照片打卡 / 查今日
│   ├── stats.js           # 统计总览
│   ├── comments.js        # 打卡帖评论：列表 / 发表 / 删除
│   └── user.js            # 资料：头像 / 昵称
